...
\begin{lemma}\label{lem:supersaturacao}
  ...
  \[
    |A| \cdot |U| \;\geq\; \frac{4(k+1)(2n+k-1)}{k},
  \]
  então
  \[
  ...
  a $|A| \cdot |U| \geq \frac{8(k+1)n}{k}$, e a cota a $e(H_{k-1}^U(A)) \geq \frac{k|A|^2|U|^2}{8n}$.
\end{lemma}
...
\begin{proof}
  ...
  Defina
  \[
    \mathcal{P} \;:=\; \bigl\{\,(x,\,(a,u_a),\,(b,u_b)) :\;
    (a,u_a) \neq (b,u_b),\; a+u_a \in I_x,\; b+u_b \in I_x\bigr\},
  \]
  o conjunto de triplas formadas por um intervalo e um par ordenado de arestas de $F$
  com ambas as pontas nesse intervalo. Fixado $x$, o número de tais pares é
  $e_F(A,I_x)(e_F(A,I_x)-1)$, de modo que
  \[
    |\mathcal{P}| \;=\; 2\sum_{x}\binom{e_F(A,I_x)}{2}. \tag{2}
  \]
  ...
  \begin{align}
    |\mathcal{P}|
    &\;\geq\; N\,\mu(\mu-1)
    \;=\; k|A||U| \cdot \left(\frac{k|A||U|}{N} - 1\right)
    \;=\; \frac{k|A||U|\,\bigl(k|A||U| - N\bigr)}{N}. \tag{3}
  \end{align}

  Para obter uma cota superior em $|\mathcal{P}|$, decompomos segundo $a = b$ ou $a \neq b$.

  \smallskip
  \noindent\textit{Caso $a = b$.}
  Neste caso $(a,u_a) \neq (b,u_b)$ força $u_a \neq u_b$, e ambos $a+u_a, a+u_b$ estão em
  $I_x$. O número de tais contribuições é
  \[
    |\mathcal{P}|_{a=b}
    \;=\; \sum_x \sum_{a \in A} N(a,I_x)\bigl(N(a,I_x)-1\bigr)
    \;\leq\; \sum_x \sum_{a \in A} N(a,I_x)^2.
  \]
  Como $|I_x| = k$, cada $N(a,I_x) \leq k$, portanto $N(a,I_x)^2 \leq k\,N(a,I_x)$. Assim:
  \[
    |\mathcal{P}|_{a=b}
    \;\leq\; k\,\sum_x e_F(A,I_x)
    \;=\; k^2|A||U|. \tag{4}
  \]

  \smallskip
  \noindent\textit{Caso $a \neq b$.}
  Seja $(a,u_a)$ e $(b,u_b)$ com $a \neq b$ ambos com $a+u_a, b+u_b \in I_x$. Como
  $I_x$ tem tamanho $k$, quaisquer dois elementos de $I_x$ têm distância no máximo $k-1$,
  portanto $|(a+u_a)-(b+u_b)| \leq k-1$, o que significa exatamente que $(u_a,u_b)$
  testemunha a aresta $\{a,b\}$ em $H_{k-1}^U(A)$.

  Reciprocamente, dado um par ordenado $((a,u_a),(b,u_b))$ com $a \neq b$ que testemunha
  uma aresta de $H_{k-1}^U(A)$ com distância $d := |(a+u_a)-(b+u_b)| \leq k-1$,
  contemos em quantos intervalos $I_x$ ambos os pontos $p := a+u_a$ e $q := b+u_b$ caem
  simultaneamente. Supondo $p < q$, necessitamos $x + 1 \leq p$ e $q \leq x + k$, isto é,
  $q - k \leq x \leq p - 1$. O número de inteiros $x$ satisfazendo isso é
  $(p-1) - (q-k) + 1 = k - d$.

  Portanto, cada par ordenado $((a,u_a),(b,u_b))$ com $a \neq b$ e distância $d$ contribui
  com $k - d \leq k$ elementos a $\mathcal{P}$. Agora, para cada aresta não-ordenada
  $\{a,b\}$ com testemunha $(u_a, u_b)$ de distância $d$, tanto o par ordenado
  $((a,u_a),(b,u_b))$ quanto o par $((b,u_b),(a,u_a))$ pertencem a $\mathcal{P}$ e cada
  um contribui $k - d$. Logo o par não-ordenado contribui com $2(k-d)$ ao total.
  Somando sobre todas as testemunhas e arestas, e usando que cada par não-ordenado
  $\{a,b\}$ com testemunha $(u_a,u_b)$ contribui via dois pares ordenados:
  \[
    |\mathcal{P}|_{a \neq b}
    \;\leq\; 2k\,e(H_{k-1}^U(A)). \tag{5}
  \]

  Combinando (4) e (5):
  \[
    |\mathcal{P}| \;\leq\; k^2|A||U| + 2k\,e(H_{k-1}^U(A)). \tag{6}
  \]

  De (3) e (6), obtemos
  \[
    k^2|A||U| + 2k\,e(H_{k-1}^U(A))
    \;\geq\; \frac{k|A||U|\,(k|A||U| - N)}{N},
  \]
  onde $N = 2n+k-1$. Dividindo por $k$ e rearranjando:
  \begin{align*}
    2\,e(H_{k-1}^U(A))
    &\;\geq\; \frac{|A||U|\,(k|A||U| - N)}{N} - k|A||U| \\
    &\;=\; |A||U| \cdot \frac{k|A||U| - (k+1)N}{N}.
  \end{align*}
  Portanto
  \[
    e(H_{k-1}^U(A))
    \;\geq\; \frac{|A||U|}{2} \cdot \frac{k|A||U| - (k+1)N}{N}. \tag{7}
  \]
  Sob a hipótese $|A||U| \geq \frac{4(k+1)N}{k}$, temos $k|A||U| \geq 4(k+1)N$, logo
  \[
    k|A||U| - (k+1)N \;\geq\; 4(k+1)N - (k+1)N \;=\; 3(k+1)N
    \;\geq\; \frac{3k|A||U|}{4} \;\geq\; \frac{k|A||U|}{2},
  \]
  onde usamos $k|A||U| \geq 4(k+1)N$ para trocar $(k+1)N$ por $\frac{k|A||U|}{4}$.
  Substituindo em (7):
  \[
    e(H_{k-1}^U(A))
  ...
  \]
  Para $k$ fixo e $n$ grande, $N = 2n+k-1 = 2n(1+o(1))$, e a cota se reduz a
  $e(H_{k-1}^U(A)) \geq \frac{k|A|^2|U|^2}{8n}$, com hipótese
  $|A||U| \geq \frac{8(k+1)n}{k}$.
\end{proof}
...
\begin{proof}
  ...
  entre si. Portanto, a quádrupla $(a_1, u_2, a_2, u_1)$ satisfaz $\{a_1,u_2\} \neq \{a_2,u_1\}$
  e
  \[
    |a_1 + u_2 - a_2 - u_1|
    \;=\; |(a_1+u_1)-(a_2+u_2)|
    \;\leq\; t-1 \;<\; t,
  \]
  contradizendo o fato de $U \cup I$ ser $t$-Sidon forte. \hfill$\blacksquare$
\end{proof}
...
\begin{proof}
  Suponha, por contradição, que existem $a_1, a_2 \in I$ com $\{a_1,a_2\}$ aresta em
  $H_{t-1}^U(A)$. Então existe $(u_1, u_2) \in U \times U$ com
  $|(a_1+u_1)-(a_2+u_2)| \leq t-1$.
  Como $I \cap U = \emptyset$, os quatro elementos $a_1, a_2, u_1, u_2$ são distintos,
  de modo que $\{a_1,u_2\} \neq \{a_2,u_1\}$. Mas
  \[
    |a_1 + u_2 - a_2 - u_1|
    \;=\; |(a_1+u_1)-(a_2+u_2)|
    \;\leq\; t-1 \;<\; t,
  \]
  contradizendo o fato de $U \cup I$ ser $t$-Sidon forte.
\end{proof}

As três proposições precedentes — multiplicidade de arestas, estrutura da cadeia e
independência — articulam-se da seguinte forma. Se $U$ é $t$-Sidon forte, então
...
Essa estrutura determina o tradeoff central do método de containers: trabalhar em $H_k^U(A)$
para $k$ maior implica mais arestas (e logo grau médio maior, containers menores), mas também
multiplicidade crescente. A tabela abaixo ilustra o caso $t = 10$, em que $k^* = 4$.
