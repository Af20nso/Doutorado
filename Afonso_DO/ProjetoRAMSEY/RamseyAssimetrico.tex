...
A fun\c{c}\~{a}o $p$ \'{e} escolhida de forma que $p(K_5) = 0$:
\[
  p(K_5) = 10 - 3 \cdot 5 + 5 = 0.
\]
Al\'{e}m disso, a condi\c{c}\~{a}o $\maxdens(G) \le 3$ implica $e(G) \le 3\,v(G)$, logo
$p(G) \le 5$; em particular, $p(G) < 0$ corresponde a grafos com densidade estritamente menor
que $\mdens(K_5) = 3$ (condi\c{c}\~{a}o do lema).

A constru\c{c}\~{a}o indutiva do bloco $B$ a partir de $H_0$ se d\'{a} atrav\'{e}s de quatro tipos de expans\~{a}o
(ver Figura~\ref{fig:expansions}), ana\'{a}logos \`{a}os de~\cite{nenadov2017algorithmic}:

\begin{itemize}
  \item \textbf{E0 (expans\~{a}o regular):} uma nova c\'{o}pia de $K_5$ \'{e} colada sobre uma aresta existente,
    adicionando $3$ novos v\'{e}rtices e $9$ novas arestas.
    Neste caso, $\Delta p = 9 - 3 \cdot 3 = 0$ (a penalidade n\~{a}o se altera).

  \item \textbf{E1 (expans\~{a}o sobre tri\^{a}ngulo):} adicionam-se $2$ novos v\'{e}rtices ligados a um tri\^{a}ngulo
    existente, criando $7$ novas arestas. Come\c{c}ando de $K_5$:
    $p = (10+7) - 3(5+2) + 5 = 17 - 21 + 5 = 1$.

  \item \textbf{E2 (expans\~{a}o sobre $K_4$):} adiciona-se $1$ novo v\'{e}rtice ligado a um $K_4$ existente,
    criando $4$ novas arestas. $\Delta p = 4 - 3 \cdot 1 = 1$.

  \item \textbf{U1 (uni\~{a}o sobre $K_5^-$):} adiciona-se $1$ nova aresta que completa um $K_5^-$
    (clique completo menos uma aresta) existente em $B$.
    $\Delta p = 1 - 3 \cdot 0 = 1$.
\end{itemize}

Observa-se que \textbf{apenas E0 preserva a penalidade}; os casos E1, E2 e U1 aumentam-na em $1$.
Como $p(K_5) = 0$ e $\maxdens(B) \le 3$ imp\~{o}e $p(B) \le 5$, o n\'{u}mero de ocorr\^{e}ncias dos
casos degenerados (E1, E2, U1) \'{e} uniformemente limitado.
